        ...
        try {
            ...
            if(quiz==null){
                ...
                for (QuestionDTO questionDTO : quiz) {
                    if(questionDTO.isResult()){
                        numCorrect++;
                    }

                }
                point = ((numCorrect + 0.0f) / quiz.size()) * 10;

                DAO dao = new DAO();
                dao.finishQuiz(quiz, quizID, numCorrect, point);
                request.setAttribute("historyID", quizID);
                request.setAttribute("numOfCorrect", numCorrect);
